...
if __name__ == "__main__":
    ...
    if optim_ == 'Adam/':
        ...
        for _ in range(num_iter):
            ...
            os.system(f"python train_lstm.py --optim_ {optim_} --lr {lr} --beta_1 {beta_1} --beta_2 {beta_2} --t_max {t_max} --seed {seed_} --model_str {model_str}  --csv_path {csv_path}")

    # Generating cases for SGD optimizer
    elif optim_ == 'SGD/':
        csv_path = "csv_files/sgd_result.csv"
        for _ in range(num_iter):
            lr = round(random.uniform(0.9,1.5), 6)
            wd = round(random.uniform(0.9,1.5), 6)
            seed_ = random.randint(1,100)
            t_max = random.randint(1,500)
            os.system(f"python train_lstm.py --optim_ {optim_}  --lr {lr} --wd {wd} --t_max {t_max} --seed {seed_} --model_str {model_str}  --csv_path {csv_path}")
    ...
    # Generating cases for AdaGrad optimizer
        ...
        for _ in range(num_iter):
            lr = round(random.uniform(0,1), 6)
            wd = round(random.uniform(0,1), 6)
            ld = round(random.uniform(0,1), 6)
            seed_ = random.randint(1,100)
            os.system(f"python train_lstm.py --optim_ {optim_}  --lr {lr} --wd {wd} --ld {ld} --seed {seed_} --model_str {model_str}  --csv_path {csv_path}")
    ...
    # Generating cases for RMSprop optimizer
        ...
        for _ in range(num_iter):
            ...
            wd = round(random.uniform(0,1), 6)
            seed_ = random.randint(1,100)
            os.system(f"python train_lstm.py --optim_ {optim_}  --lr {lr} --alpha {alpha} --wd {wd} --seed {seed_} --model_str {model_str}  --csv_path {csv_path}")
    ...
    # Generating cases for EC optimizer
        ...
        for _ in range(num_iter):
            ...
            seed_ = random.randint(1,100)
            os.system(f"python train_lstm.py --optim_ {optim_}  --alpha_0 {alpha_0}  --eta {eta} --seed {seed_} --model_str {model_str} --csv_path {csv_path}")
